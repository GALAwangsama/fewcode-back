 ...
 */
public class IndexUserContextHolder {

    private static final ThreadLocal<IndexUserContext> CONTEXT_HOLDER = new ThreadLocal<>();
    private static final ThreadLocal<UserExtraContext> EXTRA_CONTEXT_HOLDER = new ThreadLocal<>();

    private IndexUserContextHolder() {
    }
     ...
     * @param context 上下文
     */
    public static void setContext(IndexUserContext context) {
     ...
     * @param isUpdate 是否更新
     */
    public static void setContext(IndexUserContext context, boolean isUpdate) {
     ...
     * @return 上下文
     */
    public static IndexUserContext getContext() {
        IndexUserContext context = CONTEXT_HOLDER.get();
        if (null == context) {
            context = StpUtil.getSession().getModel(SaSession.USER, IndexUserContext.class);
            CONTEXT_HOLDER.set(context);
        }
        return context;
    }
     ...
     * @return 上下文
     */
    public static IndexUserContext getContext(Long userId) {
        SaSession session = StpUtil.getSessionByLoginId(userId, false);
        if (null == session) {
            return null;
        }
        return session.getModel(SaSession.USER, IndexUserContext.class);
    }
