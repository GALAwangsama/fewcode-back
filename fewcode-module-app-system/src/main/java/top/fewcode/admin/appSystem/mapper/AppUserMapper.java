...
public interface AppUserMapper extends DataPermissionMapper<AppUserDO> {
    ...
    IPage<AppUserDetailResp> selectAppUserPage(@Param("page") IPage<AppUserDO> page,
     ...
     */
    @DataPermission(tableAlias = "t1")
    List<AppUserDetailResp> selectAppUserList(@Param(Constants.WRAPPER) QueryWrapper<AppUserDO> queryWrapper);
     ...
     * @return 用户信息
     */
    @Select("SELECT * FROM sys_app_user WHERE username = #{username}")
     ...
     * @return 用户信息
     */
    @Select("SELECT * FROM sys_app_user WHERE phone = #{phone}")
     ...
     * @return 用户信息
     */
    @Select("SELECT * FROM sys_app_user WHERE email = #{email}")
     ...
     * @return 昵称
     */
    @Select("SELECT nickname FROM sys_app_user WHERE id = #{id}")
