...
import top.fewcode.admin.appSystem.service.CommonAppUserService;
...
public class AppUserServiceImpl extends BaseServiceImpl<AppUserMapper, AppUserDO, AppUserResp, AppUserDetailResp, AppUserQuery, AppUserReq> implements AppUserService, CommonAppUserService {
    ...
    public PageResp<AppUserResp> page(AppUserQuery query, PageQuery pageQuery) {
        ...
        IPage<AppUserDetailResp> page = baseMapper.selectAppUserPage(new Page<>(pageQuery.getPage(), pageQuery
    ...
    @Override
    public Boolean register(AppUserRegisterReq registerReq, String rawPassword) {
        //校验是否存在
        AppUserReq appUserReq = new AppUserReq();
        BeanUtils.copyProperties(registerReq, appUserReq);
        this.beforeAdd(appUserReq);
    ...
    protected <E> List<E> list(AppUserQuery query, SortQuery sortQuery, Class<E> targetClass) {
        ...
        List<AppUserDetailResp> entityList = baseMapper.selectAppUserList(queryWrapper);
    ...
    @Override
    public QueryWrapper<AppUserDO> buildQueryWrapper(AppUserQuery query) {
        ...
        return new QueryWrapper<AppUserDO>().and(StrUtil.isNotBlank(description), q -> q.like("t1.username", description)
                        .or()
